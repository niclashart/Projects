...
while (GPIO.input(Button) != 0):
    ...
    print(
        f"{int(position):4}, {int(Sensors[0]):4}, {int(Sensors[1]):4}, {int(Sensors[2]):4}, {int(Sensors[3]):4}, {int(Sensors[4]):4}")
    time.sleep(0.05)
Ab.forward()
...
while True:
    position, Sensors = TR.readLine()
    print(
        f"{int(position):4}, {int(Sensors[0]):3}, {int(Sensors[1]):3}, {int(Sensors[2]):3}, {int(Sensors[3]):3}, {int(Sensors[4]):3}")
    if (Sensors[0] > 900 and Sensors[1] > 900 and Sensors[2] > 900 and Sensors[3] > 900 and Sensors[4] > 900):
		...
		// improve performance.
		'''
        power_difference = proportional/30 + integral/100000 + derivative*0
